...
import NewNavigationBar from './New'
...
export default function NavigationBar({ active, onClick, openPages = [] }: NavigationBarProps) {
  const { currentTheme } = useTheme()
  const isNewTheme = currentTheme.name === 'new'

  if (isNewTheme) {
    return <NewNavigationBar active={active} onClick={onClick} openPages={openPages} />
  }
      ...
      <div className="control-dock">
        ...
        <div className="open-pages" style={{ display: 'contents' }}>
          {openPages.map((page, i) => (
            <div
              className={`control-btn ${active === page.window ? 'active' : ''}`}
              key={page.window ?? i}
